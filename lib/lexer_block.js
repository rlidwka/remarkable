...
// Seek first non-empty line from the given one and return its number
...
// Skip char codes from the given position
...
// Skip char codes in reverse from the given position
...
rules.push(function fences(state, startLine, endLine, silent) {
  var marker, len, params, nextLine, start,
  ...
  do {
    ...
    start = pos = state.bMarks[nextLine] + state.tShift[nextLine];
    ...
    // skip all markers
    for (; pos < max; pos++) {
      if (state.src.charCodeAt(pos) !== marker) { break; }
    }

    // closing code fence must be at least as long as the opening one
    if (pos - start < len) { continue; }
  ...
  state.tokens.push({
    type: 'fence',
    params: params.length ? params.split(/\s+/g) : [],
...
rules.push(function heading(state, startLine, endLine, silent) {
  ...
  // Now pos contains offset of the first non-space char
  // Let's cut tails like '    ###  ' from the end of the string
...
rules.push(function paragraph(state, startLine, endLine) {
  ...
  while (nextLine < endLine && !isEmpty(state, nextLine)) {
    // Force paragraph termination if next line is a valid tag
...
// Add function to lexer chain before one with the given name.
// Or add to the start, if name not defined
//
...
// Add function to lexer chain after one with the given name.
// Or add to the end, if name not defined
//
...
// Generate tokens for an input range
//
